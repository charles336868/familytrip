...
import { ITINERARY_DATA, CHECKLIST_DATA, TAXI_CARDS, HOTEL_INFO, SHOPPING_LIST } from './constants';
import { ItineraryView } from './components/ItineraryView';
import { ChecklistView } from './components/ChecklistView';
import { InfoView } from './components/InfoView';
import { CalendarDays, CheckSquare, Info } from 'lucide-react';

type Tab = 'itinerary' | 'checklist' | 'info';
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] text-stone-800">
      ...
      <div className="max-w-6xl mx-auto min-h-screen flex flex-col md:flex-row">
        ...
        {/* --- Main Content Area --- */}
        <main className="flex-1 p-4 md:p-8 lg:p-12 md:overflow-y-auto">
          <div className="max-w-2xl mx-auto md:mx-0">
            {activeTab === 'itinerary' && <ItineraryView data={ITINERARY_DATA} />}
            {activeTab === 'checklist' && <ChecklistView initialData={CHECKLIST_DATA} />}
            {activeTab === 'info' && <InfoView hotelInfo={HOTEL_INFO} taxiCards={TAXI_CARDS} shoppingList={SHOPPING_LIST} />}
          </div>
        </main>
      </div>
